...
from neumatic.apps.ventas.models.factura_models import Factura
...
def cargar_datos_factura():
    ...
    try:
        id_deposito_instancia = ProductoDeposito.objects.get(pk=1)
    ...
    for idx, record in enumerate(table):
        ...
        codigo_origen = int(record.get('ID', 0))  # Suponiendo que 'CODIGO' es el campo del ID en la tabla origen
        

        # Ejecutar la consulta con un marcador de posición
        nuevo_id = codigo_origen - 1
        
        sql_consult = f"UPDATE sqlite_sequence SET seq = {nuevo_id} WHERE name = 'factura';"
