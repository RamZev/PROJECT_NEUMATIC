...
class PDFRetencionView(View):
	...
	def generar_pdf_retencion(self, retencion):
		...
		proveedor = retencion.id_proveedor  # No necesitas consulta adicional
		...
		c.drawString(x_text_right, y_text_right, titulo)
		...
		nombre_empresa = f"Apellido y Nombre o Denominación: {empresa.nombre_fiscal}"
		cuit = f"C.U.I.T. Nº: {empresa.cuit_formateado}"
		domicilio = f"Domicilio: {empresa.domicilio_empresa} C.P.: {empresa.codigo_postal} - {empresa.id_localidad} - {empresa.id_provincia}"
		
		c.drawString(x_text_left, y_text_left, nombre_empresa)
		
		y_text_left -= 5*mm
		c.drawString(x_text_left, y_text_left, cuit)
		c.drawString(x_text_right, y_text_left, "Nº DE INSCRIPCIÓN: 921-7463518")
		...
		y_text_left -= 7*mm
		c.drawString(x_text_left, y_text_left, nombre_proveedor)
		
		y_text_left -= 5*mm
		c.drawString(x_text_left, y_text_left, cuit_proveedor)
		c.drawString(x_text_right, y_text_left, numero_inscripcion)
		...
		# file = f"{retencion.compro}_{retencion.letra_comprobante}_{retencion.numero_comprobante_formateado}"
		file = f"{retencion.compro}_{retencion.letra_comprobante}_{retencion.numero_comprobante}"
