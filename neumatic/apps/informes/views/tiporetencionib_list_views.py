...
class DataViewList:
	...
	table_headers = {
		...
		'descripcion_tipo_retencion_ib': (3, 'Descripción'),
		'alicuota_inscripto': (2, 'Alí. Inscripto(%)'),
		'alicuota_no_inscripto': (2, 'Alí. No Inscripto(%)'),
		'monto': (2, 'Monto'),
		'minimo': (2, 'Mínimo'),
	}
...
class TipoRetencionIbInformeListView(InformeListView):
	...
	def get_queryset(self):
		...
		if form.is_valid():
			...
			if estatus:
				match estatus:
					case "activos":
						queryset = self.model.objects.filter(estatus_tipo_retencion_ib=True)
					case "inactivos":
						queryset = self.model.objects.filter(estatus_tipo_retencion_ib=False)
					case "todos":
	...
	def get_context_data(self, **kwargs):
		...
		form = BuscadorTipoRetecionIbForm(self.request.GET or None)
...
class TipoRetencionIbInformesView(View):
	...
	def get(self, request, *args, **kwargs):
		...
		queryset_filtrado = TipoRetencionIbInformeListView()
...
class TipoRetencionIbInformePDFView(View):
