...
class DataViewList:
	...
	report_title = "Reporte de Tipo Documentos de Identidad"
	
	table_headers = {
		'estatus_tipo_documento_identidad': (1, 'Estatus'),
		'nombre_documento_identidad': (5, 'Nombre'),
...
class TipoDocumentoIdentidadInformeListView(InformeListView):
	...
	def get_queryset(self):
		...
		if form.is_valid():
			...
			if estatus:
				match estatus:
					case "activos":
						queryset = self.model.objects.filter(estatus_tipo_documento_identidad=True)
					case "inactivos":
						queryset = self.model.objects.filter(estatus_tipo_documento_identidad=False)
					case "todos":
	...
	def get_context_data(self, **kwargs):
		...
		form = BuscadorTipoDocumentoIdentidadForm(self.request.GET or None)
...
class TipoDocumentoIdentidadInformesView(View):
	...
	def get(self, request, *args, **kwargs):
		...
		queryset_filtrado = TipoDocumentoIdentidadInformeListView()
...
class TipoDocumentoIdentidadInformePDFView(View):
