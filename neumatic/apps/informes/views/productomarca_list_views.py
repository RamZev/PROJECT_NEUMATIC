...
class DataViewList:
	...
	table_headers = {
		...
		'nombre_producto_marca': (6, 'Nombre Producto Marca'),
		'principal': (1, 'Principal'),
		'info_michelin_auto': (1, 'Inf. M. auto'),
		'info_michelin_camion': (1, 'Inf. M. camión'),
		'id_moneda': (2, 'Moneda'),
	}
...
class ProductoMarcaInformeListView(InformeListView):
	...
	def get_queryset(self):
		...
		if form.is_valid():
			...
			if estatus:
				match estatus:
					case "activos":
						queryset = self.model.objects.filter(estatus_producto_marca=True)
					case "inactivos":
						queryset = self.model.objects.filter(estatus_producto_marca=False)
					case "todos":
	...
	def get_context_data(self, **kwargs):
		...
		form = BuscadorProductoMarcaForm(self.request.GET or None)
...
class ProductoMarcaInformesView(View):
	...
	def get(self, request, *args, **kwargs):
		...
		queryset_filtrado = ProductoMarcaInformeListView()
...
class ProductoMarcaInformePDFView(View):
