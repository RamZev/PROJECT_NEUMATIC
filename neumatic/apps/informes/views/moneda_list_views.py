...
class DataViewList:
	...
	table_headers = {
		...
		'nombre_moneda': (7, 'Moneda'),
		'simbolo_moneda': (1, 'Símbolo'),
		'cotizacion_moneda': (1, 'Cotización'),
		'ws_afip': (1, 'WS AFIP'),
		'predeterminada': (1, 'Predet.'),
	}
...
class MonedaInformeListView(InformeListView):
	...
	def get_queryset(self):
		...
		if form.is_valid():
			...
			if estatus:
				match estatus:
					case "activos":
						queryset = self.model.objects.filter(estatus_moneda=True)
					case "inactivos":
						queryset = self.model.objects.filter(estatus_moneda=False)
					case "todos":
	...
	def get_context_data(self, **kwargs):
		...
		form = BuscadorMonedaForm(self.request.GET or None)
...
class MonedaInformesView(View):
	...
	def get(self, request, *args, **kwargs):
		...
		queryset_filtrado = MonedaInformeListView()
...
class MonedaInformePDFView(View):
