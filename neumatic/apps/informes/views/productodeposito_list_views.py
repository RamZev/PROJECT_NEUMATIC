...
class DataViewList:
	...
	table_headers = {
		...
		'nombre_producto_deposito': (5, 'Nombre Depósito'),
	}
...
class ProductoDepositoInformeListView(InformeListView):
	...
	def get_queryset(self):
		...
		if form.is_valid():
			...
			if estatus:
				match estatus:
					case "activos":
						queryset = self.model.objects.filter(estatus_producto_deposito=True)
					case "inactivos":
						queryset = self.model.objects.filter(estatus_producto_deposito=False)
					case "todos":
						queryset = self.model.objects.all()
			
			queryset = queryset.order_by("nombre_producto_deposito")
	...
	def get_context_data(self, **kwargs):
		...
		form = BuscadorProductoDepositoForm(self.request.GET or None)
...
class ProductoDepositoInformesView(View):
	...
	def get(self, request, *args, **kwargs):
		...
		queryset_filtrado = ProductoDepositoInformeListView()
...
class ProductoDepositoInformePDFView(View):
