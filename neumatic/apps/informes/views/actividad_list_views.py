...
class DataViewList:
	...
	table_headers = {
		...
		'descripcion_actividad': (11, 'Nombre Actividad'),
	}
...
class ActividadInformeListView(InformeListView):
	...
	def get_queryset(self):
		...
		if form.is_valid():
			...
			if estatus:
				match estatus:
					case "activos":
						queryset = self.model.objects.filter(estatus_actividad=True)
					case "inactivos":
						queryset = self.model.objects.filter(estatus_actividad=False)
					case "todos":
	...
	def get_context_data(self, **kwargs):
		...
		form = BuscadorActividadForm(self.request.GET or None)
...
class ActividadInformesView(View):
	...
	def get(self, request, *args, **kwargs):
		...
		queryset_filtrado = ActividadInformeListView()
...
class ActividadInformePDFView(View):
