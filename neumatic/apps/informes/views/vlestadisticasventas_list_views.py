...
class VLEstadisticasVentasInformeView(InformeFormView):
	...
	def obtener_queryset(self, cleaned_data):
		...
		ordenar = cleaned_data.get('ordenar', None)
...
def generar_pdf(contexto_reporte):
	...
	agrupar = contexto_reporte.get("agrupar", None)
	ordenar = contexto_reporte.get("ordenar", None)
	...
	headers, blank_cols = headers_titles(agrupar, ordenar)
	...
	for obj in contexto_reporte.get("objetos", {}):
		...
		row.extend([
			formato_argentino(obj['cantidad']) if ordenar == "Cantidad" else formato_argentino(obj['total']),
			formato_argentino(obj['porcentaje_cantidad']) if ordenar == "Cantidad" else formato_argentino(obj['porcentaje_total'])
		])
...
def headers_titles(agrupar, ordenar):
	headers = {}
	blank_cols = []
	...
	if agrupar == "Producto":
		headers = {
			"id_producto_id": (40, "Código"),
			"cai": (70, "CAI"),
			"nombre_producto": (150, "Descripción"),
	...
	headers.update({
		"nombre_producto_marca": (120, "Marca"),
	})
