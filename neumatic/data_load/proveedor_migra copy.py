...
def precargar_relaciones():
    ...
    relaciones = {
        ...
        'retenciones_ib': defaultdict(
            lambda: None,
            {r.pk: r for r in TipoRetencionIb.objects.all()}
        ),
    ...
    try:
        relaciones['default_tipo_retencion_ib'] = TipoRetencionIb.objects.get(pk=1)
...
def procesar_lote(records, cache, batch_start):
    ...
    for idx, record in enumerate(records):
        try:
            ...
            tipo_iva = cache['tipos_iva'][record.get('IVA', '').strip()] or cache['default_tipo_iva']
            retencion_ib = cache['retenciones_ib'].get(int(record.get('IDRETENCION', 0)), cache['default_tipo_retencion_ib'])
            ...
            if not tipo_iva:
                ...
                continue
            if not retencion_ib:
                logger.warning(f"Tipo Retención IB no válido para registro {record.get('CODIGO')}. Registro omitido.")
                continue
            
            # Localidad y provincia
            localidad_value = str(record.get('LOCALIDAD', ''))[:5].strip() or None
            ...
            cuit = int(str(record.get('CUIT', 0)).strip()) if record.get('CUIT') else 0
            ...
            ib_numero = record.get('INGBRUTO', '').strip() or "000000000000000"
            ...
            proveedor = Proveedor(
                ...
                nombre_proveedor=record.get('NOMBRE', '').strip()[:50],
                domicilio_proveedor=record.get('DOMICILIO', '').strip()[:50],
                codigo_postal=str(record.get('CODPOSTAL', ''))[:5].strip() or "00000",
                ...
                telefono_proveedor=record.get('TELEFONO', '').strip()[:15] or "",
                movil_proveedor=None,  # Asignado como None según reglas
                email_proveedor=record.get('MAIL', '').strip()[:50] or "",
                observacion_proveedor=record.get('NOTA', '').strip() or ""
            )
