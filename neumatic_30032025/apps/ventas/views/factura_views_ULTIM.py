	...
	def form_valid(self, form):
		...
		try:
				with transaction.atomic():
						# 1. Extracción y validación simplificada
						...
						deposito = form.cleaned_data['id_deposito']
						fecha_comprobante = form.cleaned_data['fecha_comprobante']

						# 2. Bloqueo y actualización más eficiente
						...
						# 3. Incremento atómico directo (sin verificación redundante)
						nuevo_numero = numero_obj.numero + 1
						Numero.objects.filter(pk=numero_obj.pk).update(numero=F('numero') + 1)
						
						# 4. Asignación y validación
						form.instance.numero_comprobante = nuevo_numero
						form.instance.full_clean()  # Validación general del modelo

						# 5. Guardado en cascada
						...
						# 6. Actualización de stock para cada producto en el detalle
						# for detalle in detalles:
						# 		if detalle.act_stock and detalle.id_producto and deposito:
						# 				# Bloqueo y actualización atómica del stock
						# 				ProductoStock.objects.select_for_update(nowait=True).filter(
						...
						messages.success(
								self.request, 
								f"Factura {nuevo_numero} creada correctamente"
						)
				...
				messages.error(self.request, f"Error en datos: {', '.join(e.messages)}")
				return self.form_invalid(form)
