...
def cargar_datos():
    ...
    for idx, record in enumerate(table):
        ...
        # Vendedor
        vendedor_id = record.get('VENDEDOR', 0)
        if vendedor_id:
            try:
                id_vendedor_instancia = Vendedor.objects.get(pk=vendedor_id)
            except Vendedor.DoesNotExist:
                id_vendedor_instancia = None
        else:
            id_vendedor_instancia = None
         ...
         # Actividad
        actividad_id = record.get('ACTIVIDAD', 0)
        if actividad_id:
            try:
                id_actividad_instancia = Actividad.objects.get(pk=actividad_id)
            except (Actividad.DoesNotExist, ValueError):
                id_actividad_instancia = None
        else:
            id_actividad_instancia = None
        ...
        # Sucursal
        sucursal_id = record.get('SUCURSAL', 0)
        if sucursal_id:
            try:
        ...
        # Percepción IB
        percepcion_ib_id = record.get('PERCEPIB', 0)
        if percepcion_ib_id:
            try:
                id_percepcion_ib_instancia = TipoPercepcionIb.objects.get(pk=percepcion_ib_id)
            except (TipoPercepcionIb.DoesNotExist, ValueError):
                id_percepcion_ib_instancia = None
        else:
            id_percepcion_ib_instancia = None
